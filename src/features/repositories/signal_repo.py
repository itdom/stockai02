...
class KdjCrossSignalRepository:
    def __init__(self, writer: RecordWriter) -> None:
        self.writer = writer
        self.table = get_table_spec("signal_kdj_cross")

    def save_signals(self, records: list[Mapping[str, Any]]) -> int:
        aligned = align_records(records, self.table)
        if not aligned:
            return 0
        return self.writer.upsert(self.table, aligned)

    def delete_signals_window(
        self,
        *,
        start_date: str,
        end_date: str,
        ...
        equals: dict[str, Any] = {"frequency": frequency}
